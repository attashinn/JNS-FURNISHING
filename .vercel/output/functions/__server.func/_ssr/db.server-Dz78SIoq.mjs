...
//#region node_modules/.nitro/vite/services/ssr/assets/db.server-Dz78SIoq.js
var _sql = null;
...
var memDb = {
	...
	reviews: [
		{
			id: 1,
			product_slug: "luxury-blackout-curtain",
			user_id: 1,
			user_name: "Farhana R.",
			rating: 5,
			title: "Total Room Blackout & Beautiful Drape",
			body: "Ordered 6 panels for our Gulshan apartment. Blocks the afternoon Dhaka heat completely and looks magnificent.",
			created_at: (/* @__PURE__ */ new Date()).toISOString()
		},
		{
			id: 2,
			product_slug: "belgian-linen-sheer",
			user_id: 1,
			user_name: "Zubair H.",
			rating: 5,
			title: "Natural Sunlight Filtration",
			body: "The linen texture is top tier. Gives our living room a breezy Scandinavian luxury feel.",
			created_at: (/* @__PURE__ */ new Date()).toISOString()
		},
		{
			id: 3,
			product_slug: "egyptian-cotton-bedding-set",
			user_id: 1,
			user_name: "Tahmina S.",
			rating: 5,
			title: "Silky Hotel Grade Comfort",
			body: "Extraordinarily soft and stays cool all night. The 400TC quality is noticeable immediately.",
			created_at: (/* @__PURE__ */ new Date()).toISOString()
		},
		{
			id: 4,
			product_slug: "sofa-cover-series",
			user_id: 1,
			user_name: "Mahir A.",
			rating: 5,
			title: "Saved our 3-Seater Sofa",
			body: "Fits like custom upholstery. The textured stretch waffle fabric doesn't slip at all.",
			created_at: (/* @__PURE__ */ new Date()).toISOString()
		}
	],
	nextId: {
...
function createMemorySql() {
	return async function sql(strings, ...values) {
		const normalized = strings.reduce((acc, str, i) => acc + str + (i < values.length ? `$${i + 1}` : ""), "").trim().replace(/\s+/g, " ").trim();
		if (/^(CREATE TABLE|ALTER TABLE|CREATE INDEX)/i.test(normalized)) return [];
		if (/SELECT COUNT\(\*\)::int AS c FROM products/i.test(normalized)) return [{ c: memDb.products.length }];
		if (/INSERT INTO products/i.test(normalized)) {
			...
			if (existingIdx >= 0) {
				if (!/ON CONFLICT/i.test(normalized)) memDb.products[existingIdx] = {
		...
		if (/FROM products WHERE slug =/i.test(normalized)) {
			...
			return found ? [{
				...found,
				ugc_videos: found.ugc_videos ?? []
			}] : [];
		}
		if (/FROM products/i.test(normalized)) return [...memDb.products].sort((a, b) => a.id - b.id).map((p) => ({
			...p,
			ugc_videos: p.ugc_videos ?? []
		}));
		if (/SELECT product_slug AS slug, AVG\(rating\)/i.test(normalized)) {
			...
			for (const r of memDb.reviews) {
				if (!grouped[r.product_slug]) grouped[r.product_slug] = {
					total: 0,
					count: 0
				};
		...
		if (/FROM reviews WHERE product_slug =/i.test(normalized)) {
			const targetSlug = values[0];
			return memDb.reviews.filter((r) => r.product_slug === targetSlug).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).slice(0, 50);
		}
		if (/FROM reviews/i.test(normalized)) return [...memDb.reviews].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
		...
		if (/FROM verification_codes/i.test(normalized)) {
			...
			return memDb.verification_codes.filter((c) => c.email === email && c.code === code && !c.used && c.expires_at > now).sort((a, b) => b.id - a.id).slice(0, 1);
		}
...
async function ensureDb() {
    ...
    `;
		...
		if ((await sql`SELECT COUNT(*)::int AS c FROM products`)[0]?.c === 0) for (const p of SEED_PRODUCTS) await sql`
...
//#endregion
export { SEED_PRODUCTS, ensureDb, getSql };
